...
This is a pipeline that generates a mock data vector for a 6$\times$2pt observation - the joint combination of galaxy 
weak lensing/cosmic shear, photometric galaxy clustering, and CMB lensing. From a fiducial cosmology, we:

<ul>
<li>generate correlated (tomographic) realisations of the 6$\times$2pt signal from their harmonic power spectra, using 
...
<li>measure Pseudo-$C_{\ell}$ bandpowers for the 6$\times$2pt signal over a given survey footprint (using `NaMaster`, 
<https://github.com/LSSTDESC/NaMaster>, Alonso et al. 2019)</li>
<li>propagate the measured 6$\times$2pt bandpowers into cosmological parameter constraints on $w_{0}w_{a}$CDM. This 
includes:
...
<li>a Gaussian likelihood analysis to derive Bayesian parameter constraints on $w_{0}w_{a}$CDM - performed using nested
